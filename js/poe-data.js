/* ============================================================
  ...
  const POE_EXAMPLES = [
    ...
    },
    {
      ...
      scene: { floors: [
          { key: 'S1', x1: 34, y1: 49, x2: 46, y2: 61, smooth: true },
          { key: 'G',  x1: 46, y1: 61, x2: 66, y2: 61, smooth: true },
          { key: 'S2', x1: 66, y1: 61, x2: 90, y2: 49, smooth: true } ],
        elements: [
      ...
      observe: '▶ 실행 후 v–t 그래프를 보세요. 두 곡선의 기울기는 다르지만 바닥에 닿을 때의 높이(최댓값)는 같습니다. 빗면과 바닥의 이음은 클로소이드로 다듬어져 튀지 않습니다(바닥면을 클릭해 토글). 바닥에서 만나 충돌하는 것도 지켜보세요.',
      vis: ['graph:v', 'vectors'], select: 'A',
    ...
    {
      id: 'joint-bounce', cat: 'circular', title: '빗면 끝 모서리를 지나는 공', level: '심화',
      misconception: '면이 이어져 있으면 물체는 그냥 따라간다 (모서리의 충격력을 생각하지 않음)',
      scene: { floors: [
          { key: 'S', x1: 38, y1: 62, x2: 62, y2: 50, isFriction: true, muS: 0.6, muK: 0.5 },
          { key: 'G', x1: 20, y1: 62, x2: 38, y2: 62, isFriction: true, muS: 0.6, muK: 0.5 } ],
        elements: [{ key: 'A', type: 'circle', mass: 1, e: 1, onFloor: { floor: 'S', x: 56 }, showTrail: true }], view: 'fit' },
      question: '27° 빗면을 굴러 내려온 공(반발계수 1)이 수평면과 만나는 날카로운 모서리를 지납니다. 공은?',
      options: [
        { label: '튀어 오른다 — 모서리에서 충격력을 받는다', correct: true },
        { label: '방향만 바꿔 그대로 매끄럽게 굴러간다', tag: '접선이 꺾이는 곳에서는 속도의 법선 성분 v·sinθ 가 순간에 사라져야 함을 놓침' },
        { label: '모서리에 걸려 멈춘다', tag: '충격의 방향을 잘못 본 경우 — 충격은 새 면의 법선 방향' },
      ],
      observe: '▶ 실행 후 공이 튀는 높이를 보세요. 비교표의 "매끄러운 이음" 줄을 누르면 두 바닥면 사이에 클로소이드가 들어가 튀지 않습니다. 바닥면을 클릭하면 이음 토글과 곡률 반지름이 나옵니다.',
      vis: ['vectors', 'forces', 'graph:vy'], select: 'A',
      variants: [{ label: '날카로운 모서리 (기준)' }, { label: '매끄러운 이음 (클로소이드)', set: { S: { smoothP1: true }, G: { smoothP2: true } } }],
      measure: { body: 'A', q: 'vy', stat: 'max', T: 4, label: '모서리 지난 뒤 최대 상승 속도', unit: 'm/s' },
      explain: '모서리에서는 접선 방향이 27° 꺾이므로 속도의 새 법선 성분 v sin27° ≈ 4 m/s 가 순간에 0 이 되어야 합니다. 그 순간 힘은 무한대 × 0 초의 충격력이고, 반발계수 1 이면 그대로 되튑니다. 접선이 연속인 곡선(원호)으로 이으면 충격은 없어지지만 원호에 들어서는 순간 수직항력이 mv²/R 만큼 뛰고, 곡률까지 연속인 클로소이드로 이으면 수직항력도 부드럽게 변합니다. 매끄럽게 이어진 면 위에서는 법선 속도가 원래 0 이라 튈 것이 없습니다.',
    },
    {
      id: 'hill-leave', cat: 'circular', title: '둥근 언덕 꼭대기에서 떠오를까', level: '심화',
      misconception: '언덕을 넘을 만큼 빠르면 늘 면을 따라 넘어간다',
      scene: 'hill',
      question: '13 m/s 로 달려온 공이 꼭대기(높이 약 7.2 m)를 둥글게 다듬은 언덕(꼭대기 곡률 반지름 R ≈ 5.1 m)을 넘습니다. 꼭대기에서 공은 면을 따라갈까요, 떠오를까요?',
      options: [
        { label: '13 m/s 면 면을 따라 넘지만, 더 빠르면 꼭대기 부근에서 떠오른다 (v_top² > g(R + r))', correct: true },
        { label: '넘을 만큼 빠르면 언제나 면을 따라간다', tag: '볼록한 면에서는 수직항력 N = mg cosθ − mv²/R 이 0 이 되면 떠난다는 것을 놓침' },
        { label: '꼭대기에 닿기 전에 항상 떠오른다', tag: '떠오르는 조건은 속력에 달려 있다 — 느리면 면을 따라간다' },
      ],
      observe: '▶ 실행 후 꼭대기 근처 궤적과 힘 표시의 수직항력을 보세요. 비교표에서 15·18 m/s 로 바꾸면 꼭대기 전에 궤적이 면을 떠나 포물선이 됩니다(공중 체류 시간이 0 이 아니게 됨). 바닥면을 클릭하면 꼭대기 이음의 R 이 나옵니다.',
      vis: ['forces', 'vectors', 'graph:y'], select: 'A',
      variants: [{ label: '13 m/s (기준)' }, { label: '15 m/s', set: { A: { vx0: 15 } } }, { label: '18 m/s', set: { A: { vx0: 18 } } }],
      measure: { body: 'A', q: 'air', stat: 'sum', T: 3.8, label: '언덕을 넘는 동안 면을 떠나 있던 시간', unit: 's' },
      explain: '볼록한 면 위에서 물체가 면을 따라 돌려면 중심을 향한 알짜힘 mv²/R 이 필요합니다: mg cosθ − N = mv²/R. 꼭대기(θ = 0)에서 N = mg − mv²/R 이고, v² > gR 이면 N 이 음수가 되어야 하므로 면이 잡아 줄 수 없어 떠오릅니다. 공의 중심은 반지름 R + r ≈ 5.6 m 인 궤도를 돌므로 한계 속력은 √(g(R + r)) ≈ 7.4 m/s 입니다. 13 m/s 로 출발하면 꼭대기 속력은 √(13² − 2g·7.2) ≈ 5.2 m/s 로 조건을 만족해 면을 따라가고(체류 시간 0), 15 m/s 면 꼭대기 속력이 ≈ 9 m/s 로 한계를 넘어 꼭대기에 닿기 전에 떠납니다.',
    },
  ];
  ...
  const POE_GUIDES = [
    ...
    { scene: 'hill', title: '둥근 언덕 넘기', vis: ['forces', 'vectors', 'graph:y'], select: 'A',
      steps: [
        '바닥면을 클릭해 이음 행을 보세요 — 네 이음이 모두 "매끄럽게 ON" 이고 꼭대기 이음의 곡률 반지름 R 이 적혀 있습니다.',
        '실행하며 힘 표시의 수직항력을 보세요. 오목 이음에서는 커지고(mv²/R 이 더해짐), 볼록한 꼭대기에서는 작아집니다(빠짐).',
        '초기 속도를 15·18 m/s 로 올려 다시 실행하세요 — 꼭대기 전에 N 이 0 이 되어 공이 면을 떠나 포물선을 그립니다. 이음을 OFF 로 바꾸면 날카로운 꼭대기에서 곧바로 떠오릅니다.' ],
      why: '곡면 위에서는 중심 방향 알짜힘 mv²/R 이 필요합니다. 오목한 면은 수직항력을 키워 이를 대고, 볼록한 면은 중력에서 빼서 댑니다. 빼도 모자라면(v² > gR cosθ) 면을 떠납니다. 클로소이드 이음은 곡률이 연속이라 수직항력이 계단 없이 변합니다.' },
  ];
