...
![Tests](https://img.shields.io/badge/%EB%AC%BC%EB%A6%AC%20%EA%B2%80%EC%A6%9D-242%20%2B%2041-16a34a?style=flat-square)
...
보여 줍니다. 학생이 먼저 예측하고 확인하는 **POE 활동 32개**, 변수를 바꿔 가며 규칙을 찾는 **파라미터 스윕**,
...
처음 열면 **시작 카드**에 대표 장면 11개가 떠서 한 번 클릭으로 시작할 수 있고,
...
| 🧱 요소 | 👁 보기 | 📚 수업 |
|---|---|---|
| 네모·원 물체(질량·초기 속도·반발계수·공기저항·초기 각속도), 바닥면(직선·꺾임·곡선·마찰), 힘 구간, 외력, 실, 도르래(고정·움직), 용수철 | 속도 벡터 · **자유물체도**(중력·수직항력·마찰력·장력·알짜힘) · 속성 패널 측정값 · **y·x·v·a·E–t 그래프** · 직전 실행 잔상 비교 · 느린 배속·한 스텝 | **POE 32문항**(7분류·오개념 태그·비교표·CSV) · 해설 카드 11 · **탐구 카드 10** · **파라미터 스윕** · 공유 링크 · 수능 규격 SVG 촬영 |
...
| **📚 예제** | 대표 장면 11개 — 포물선·원운동·회전 낙하 포함 |
| **POE** | 예측·관찰·설명 32문항 + 해설 11 + 탐구 카드 10 |
...
```bash
node test/run-all.js          # 물리·모듈 수치 검증 — 16 스위트 242 항목 (Node, 의존성 없음)
